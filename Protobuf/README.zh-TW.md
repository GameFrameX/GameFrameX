...
| Proto 檔案 | 模組 | 說明 |
|------------|------|------|
| `_-0002_Inner_Basic.proto` | -2 | 內部基礎協議（僅伺服器） |
...
- 檔名：對外 `_<ModuleID:0000>_<Domain>.proto`（如 `_0500_Mail.proto`）；內部 `_-<ModuleID:0000>_Inner_<Domain>.proto`（如 `_-0120_Inner_Social.proto`）。模組 ID **對外為正**（客戶端 ↔ 伺服器）、**內部為負**（伺服器 ↔ 伺服器）；負數 ID 在檔名裡保留負號，所有檔名都以 `_` 開頭，既保證合法（不以 `-` 開頭），又統一排序。
- 內部模組號與對外模組號**鏡像取號**：存在對外對應模組時取其負值（`_0120_Social` 120 ↔ `_-0120_Inner_Social` -120）；無對外對應時從內部保留段取號（如 `_-0002_Inner_Basic` -2）。
- `-s`/`_s` 檔名後綴機制**在本倉禁用**（匯出工具仍支援，但本倉任何檔案不得使用）。

**為什麼** —— 把模組 ID 寫進檔名，檔名本身就是路由鍵：一眼能看出屬於哪個業務域，兩個檔案也絕不可能悄悄佔用同一個號。負模組號讓內部協議在匯出時被自動過濾，不會洩露給客戶端。
...
> **關於當前倉庫的說明：** 這裡的內部檔案統一採用 `_-<ModuleID>_Inner_<Domain>.proto` 加**負模組 ID** 的唯一約定（如 `_-0120_Inner_Social.proto`、`_-0002_Inner_Basic.proto`）。`-s`/`_s` 後綴機制在本倉禁用，僅伺服器路由一律透過負模組 ID 實現。
