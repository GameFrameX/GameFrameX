...
| Proto 파일 | 모듈 | 설명 |
|------------|------|------|
| `_-0002_Inner_Basic.proto` | -2 | 내부 기본 프로토콜(서버 전용) |
...
- 파일명: 외부는 `_<ModuleID:0000>_<Domain>.proto`(예: `_0500_Mail.proto`), 내부는 `_-<ModuleID:0000>_Inner_<Domain>.proto`(예: `_-0120_Inner_Social.proto`). 모듈 ID는 **외부는 양수**(클라이언트 ↔ 서버), **내부는 음수**(서버 ↔ 서버). 음수 ID는 파일명에 부호를 그대로 남기고, 모든 파일명이 `_`로 시작하므로 합법(`-`로 시작하지 않음)이며 정렬도 통일됩니다.
- 내부 모듈 번호는 외부 모듈과 **미러 채번**합니다: 외부 대응 모듈이 있으면 그 음수값(`_0120_Social` 120 ↔ `_-0120_Inner_Social` -120), 없으면 내부 예약 대역에서 채번(예: `_-0002_Inner_Basic` -2).
- `-s`/`_s` 파일명 접미사 방식은 **본 리포지토리에서 사용 금지**입니다(내보내기 도구는 계속 지원하지만 본 리포지토리의 어떤 파일도 사용할 수 없음).

**이유** —— 모듈 ID를 파일명에 넣으면 파일명 자체가 라우팅 키가 됩니다: 도메인을 한눈에 알 수 있고, 두 파일이 몰래 같은 번호를 공유할 수도 없습니다. 음수 모듈 번호 덕분에 내부 프로토콜은 내보낼 때 자동으로 걸러지고, 클라이언트에 유출되지 않습니다.
...
> **현재 리포지토리 참고:** 여기서 내부 파일은 `_-<ModuleID>_Inner_<Domain>.proto`와 **음수 모듈 ID**를 함께 사용하는 유일한 규칙으로 통일됩니다(예: `_-0120_Inner_Social.proto`, `_-0002_Inner_Basic.proto`). `-s`/`_s` 접미사 방식은 본 리포지토리에서 사용이 금지되며, 서버 전용 라우팅은 모두 음수 모듈 ID로 구현합니다.
