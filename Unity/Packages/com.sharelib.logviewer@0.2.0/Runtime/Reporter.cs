...
public class Reporter : MonoBehaviour
{
    ...
    void addSample()
    {
        ...
        sample.loadedScene = (byte) ActiveScene.buildIndex;
#else
    ...
    private const int GCValue = 1024 * 1024;

    void Update()
    {
        fpsText = fps.ToString("0.000");
        gcTotalMemory = (((float) System.GC.GetTotalMemory(false)) /GCValue );
        //addSample();
        var ActiveScene = SceneManager.GetActiveScene();
#if UNITY_CHANGE3
        int sceneIndex = ActiveScene.buildIndex;
        if (sceneIndex != -1 && string.IsNullOrEmpty(scenes[sceneIndex]))
            scenes[ActiveScene.buildIndex] = ActiveScene.name;
#else
