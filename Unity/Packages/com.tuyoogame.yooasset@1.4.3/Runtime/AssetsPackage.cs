    ...
    public class AssetsPackage
    {
        ...
        public InitializationOperation InitializeAsync(InitializeParameters parameters)
        {
            ...
            if (_playMode == EPlayMode.EditorSimulateMode)
            {
                ...
                _assetSystemImpl.Initialize(PackageName, true, parameters.AssetLoadingMaxNumber,
                    parameters.DecryptionServices, _bundleServices);

                var initializeParameters = parameters as EditorSimulateModeParameters;
                initializeOperation = editorSimulateModeImpl.InitializeAsync(initializeParameters.LocationToLower,
            ...
            else if (_playMode == EPlayMode.OfflinePlayMode)
            {
                ...
                _assetSystemImpl.Initialize(PackageName, false, parameters.AssetLoadingMaxNumber,
                    parameters.DecryptionServices, _bundleServices);

                var initializeParameters = parameters as OfflinePlayModeParameters;
                initializeOperation =
                    offlinePlayModeImpl.InitializeAsync(PackageName, initializeParameters.LocationToLower);
            }
        ...
        private void CheckInitializeParameters(InitializeParameters parameters)
        {
            ...
            if (parameters is EditorSimulateModeParameters)
            {
                ...
                if (string.IsNullOrEmpty(editorSimulateModeParameters.SimulatePatchManifestPath))
                    throw new Exception(
                        $"{nameof(editorSimulateModeParameters.SimulatePatchManifestPath)} is null or empty.");
            }
        ...
        public SceneOperationHandle LoadSceneAsync(string location, LoadSceneMode sceneMode = LoadSceneMode.Single,
        ...
        /// <param name="activateOnLoad">加载完毕时是否主动激活</param>
        /// <param name="priority">优先级</param>
        public SceneOperationHandle LoadSceneAsync(AssetInfo assetInfo, LoadSceneMode sceneMode = LoadSceneMode.Single,
            bool activateOnLoad = true, int priority = 100)
        {
            DebugCheckInitialize();
            var handle = _assetSystemImpl.LoadSceneAsync(assetInfo, sceneMode, activateOnLoad, priority);
        ...
        {
            DebugCheckInitialize();
            return _playModeServices.CreatePatchDownloaderByTags(new string[] {tag}, downloadingMaxNumber,
        ...
        /// <param name="failedTryAgain">下载失败的重试次数</param>
        /// <param name="timeout">超时时间</param>
        public PatchDownloaderOperation CreatePatchDownloader(string[] tags, int downloadingMaxNumber,
        ...
        /// <param name="failedTryAgain">下载失败的重试次数</param>
        /// <param name="timeout">超时时间</param>
        public PatchDownloaderOperation CreateBundleDownloader(AssetInfo[] assetInfos, int downloadingMaxNumber,
            int failedTryAgain, int timeout = 60)
        {
            DebugCheckInitialize();
        ...
        {
            DebugCheckInitialize();
            return _playModeServices.CreatePatchUnpackerByTags(new string[] {tag}, unpackingMaxNumber, failedTryAgain,
                int.MaxValue);
        }
        ...
        private void DebugCheckUpdateManifest()
        {
            ...
            {
                YooLogger.Warning(
                    $"Found loaded bundle before update manifest ! Recommended to call the  {nameof(ForceUnloadAllAssets)} method to release loaded bundle !");
            }
        }
