            ...
            try
            {
                if (_steps == ESteps.DeserializeFileHeader)
                {
                    ...
                    if (fileVersion != YooAssetSettings.PatchManifestFileVersion)
                    {
                        ...
                        Status = EOperationStatus.Failed;
                        Error =
                            $"The manifest file version are not compatible : {fileVersion} != {YooAssetSettings.PatchManifestFileVersion}";
                        return;
                    }
                ...
                if (_steps == ESteps.DeserializeBundleList)
                {
                    while (_patchBundleCount > 0)
                    {
                        ...
                        _patchBundleCount--;
                        Progress = 1f - _patchBundleCount / (_progressTotalValue * 1f);
                        if (OperationSystem.IsBusy)
                            break;
                    }
