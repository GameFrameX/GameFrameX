...
namespace YooAsset.Editor
{
    ...
    public static class EditorTools
    {
        ...
        {
            EditorUtility.DisplayProgressBar("进度", $"{tips} : {progressValue}/{totalValue}",
        ...
        public static void FileRename(string filePath, string newName)
        {
            ...
            if (Path.HasExtension(filePath))
            {
                ...
                destPath = $"{dirPath}/{newName}{extension}";
            }
            else
            {
        ...
        public static bool CheckBundleFileValid(byte[] fileData)
        {
            ...
            if (signature == "UnityFS" || signature == "UnityRaw" || signature == "UnityWeb" ||
                signature == "\xFA\xFA\xFA\xFA\xFA\xFA\xFA\xFA")
                return true;
            else
                return false;
        }
        ...
        private static string Substring(string content, string key, bool includeKey, bool firstMatch = true)
        {
            ...
            int startIndex = -1;
            if (firstMatch)
                startIndex = content.IndexOf(key); //返回子字符串第F一次出现位置		
            else
