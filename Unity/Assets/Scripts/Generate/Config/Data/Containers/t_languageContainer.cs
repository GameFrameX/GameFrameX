...
namespace Geek.Client.Config
{
    ...
    public class t_languageContainer : BaseContainer
    {
        ...
        public override void loadDataFromBin()
        {
            ...
            var fileOperationHandle =
                AssetComponent.Instance.LoadRawFileSync(
                    AssetUtility.GetConfigPath($"t_language{useField.Replace("t_", "")}Bean"));
            byte[] data = fileOperationHandle.GetRawFileData();
            if (data != null)
            {
                try
                {
