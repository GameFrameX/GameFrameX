...
namespace cfg.test
{
    public sealed partial class TbTestIndex
    {
        ...
        public TbTestIndex(JSONNode _json)
        {
            var count = _json.Children.Count();
            _dataMap = new Dictionary<int, test.TestIndex>(count);
            _dataList = new List<test.TestIndex>(count);
            
            foreach(var _row in _json.Children)
            {
                var _v = test.TestIndex.DeserializeTestIndex(_row);
                _dataList.Add(_v);
