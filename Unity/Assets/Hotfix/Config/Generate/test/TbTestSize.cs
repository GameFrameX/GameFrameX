...
namespace cfg.test
{
    public sealed partial class TbTestSize
    {
        private readonly Dictionary<int, test.TestSize> _dataMap;
        private readonly List<test.TestSize> _dataList;
        
        public TbTestSize(JSONNode _json)
        {
            var count = _json.Children.Count();
            _dataMap = new Dictionary<int, test.TestSize>(count);
            _dataList = new List<test.TestSize>(count);
            
            foreach(var _row in _json.Children)
            {
                var _v = test.TestSize.DeserializeTestSize(_row);
                _dataList.Add(_v);
