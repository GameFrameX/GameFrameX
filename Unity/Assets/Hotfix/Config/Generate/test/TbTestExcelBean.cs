//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
using Bright.Serialization;
using System.Collections.Generic;
using System.Linq;
using SimpleJSON;

namespace cfg.test
{
    public sealed partial class TbTestExcelBean
    {
        private readonly Dictionary<int, test.TestExcelBean1> _dataMap;
        private readonly List<test.TestExcelBean1> _dataList;
        
        public TbTestExcelBean(JSONNode _json)
        {
            var count = _json.Children.Count();
            _dataMap = new Dictionary<int, test.TestExcelBean1>(count);
            _dataList = new List<test.TestExcelBean1>(count);
            
            foreach(var _row in _json.Children)
            {
                var _v = test.TestExcelBean1.DeserializeTestExcelBean1(_row);
                _dataList.Add(_v);
                _dataMap.Add(_v.X1, _v);
            }
            PostInit();
        }
    
        public Dictionary<int, test.TestExcelBean1> DataMap => _dataMap;
        public List<test.TestExcelBean1> DataList => _dataList;
    
        public test.TestExcelBean1 GetOrDefault(int key)
        {
            return _dataMap.TryGetValue(key, out var v) ? v : null;
        }
        
        public test.TestExcelBean1 Get(int key)
        {
            return _dataMap[key];
        }
        
        public test.TestExcelBean1 this[int key]
        {
            get { return _dataMap[key]; }
        }
    
        public void Resolve(Dictionary<string, object> _tables)
        {
            foreach(var v in _dataList)
            {
                v.Resolve(_tables);
            }
            PostResolve();
        }
    
        public void TranslateText(System.Func<string, string, string> translator)
        {
            foreach(var v in _dataList)
            {
                v.TranslateText(translator);
            }
        }
        
        
        partial void PostInit();
        partial void PostResolve();
    }
}