//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
using Bright.Serialization;
using System.Collections.Generic;
using System.Linq;
using SimpleJSON;

namespace cfg.item
{
    public sealed partial class TbItemExtra
    {
        private readonly Dictionary<int, item.ItemExtra> _dataMap;
        private readonly List<item.ItemExtra> _dataList;
        
        public TbItemExtra(JSONNode _json)
        {
            var count = _json.Children.Count();
            _dataMap = new Dictionary<int, item.ItemExtra>(count);
            _dataList = new List<item.ItemExtra>(count);
            
            foreach(var _row in _json.Children)
            {
                var _v = item.ItemExtra.DeserializeItemExtra(_row);
                _dataList.Add(_v);
                _dataMap.Add(_v.Id, _v);
            }
            PostInit();
        }
    
        public Dictionary<int, item.ItemExtra> DataMap => _dataMap;
        public List<item.ItemExtra> DataList => _dataList;
    
        public T GetOrDefaultAs<T>(int key) where T : item.ItemExtra => _dataMap.TryGetValue(key, out var v) ? (T)v : null;
        public T GetAs<T>(int key) where T : item.ItemExtra => (T)_dataMap[key];
        public item.ItemExtra GetOrDefault(int key)
        {
            return _dataMap.TryGetValue(key, out var v) ? v : null;
        }
        
        public item.ItemExtra Get(int key)
        {
            return _dataMap[key];
        }
        
        public item.ItemExtra this[int key]
        {
            get { return _dataMap[key]; }
        }
    
        public void Resolve(Dictionary<string, object> _tables)
        {
            foreach(var v in _dataList)
            {
                v.Resolve(_tables);
            }
            PostResolve();
        }
    
        public void TranslateText(System.Func<string, string, string> translator)
        {
            foreach(var v in _dataList)
            {
                v.TranslateText(translator);
            }
        }
        
        
        partial void PostInit();
        partial void PostResolve();
    }
}