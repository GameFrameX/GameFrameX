...
namespace cfg.item
{
    public sealed partial class TreasureBox :  item.ItemExtra 
    {
        public TreasureBox(JSONNode _json)  : base(_json) 
        {
            ...
            { var __json0 = _json["drop_ids"]; if(!__json0.IsArray) { throw new SerializationException(); } DropIds = new System.Collections.Generic.List<int>(__json0.Count); foreach(JSONNode __e0 in __json0.Children) { int __v0;  { if(!__e0.IsNumber) { throw new SerializationException(); }  __v0 = __e0; }  DropIds.Add(__v0); }   }
            { var __json0 = _json["choose_list"]; if(!__json0.IsArray) { throw new SerializationException(); } ChooseList = new System.Collections.Generic.List<item.ChooseOneBonus>(__json0.Count); foreach(JSONNode __e0 in __json0.Children) { item.ChooseOneBonus __v0;  { if(!__e0.IsObject) { throw new SerializationException(); }  __v0 = item.ChooseOneBonus.DeserializeChooseOneBonus(__e0);  }  ChooseList.Add(__v0); }   }
            PostInit();
        }
