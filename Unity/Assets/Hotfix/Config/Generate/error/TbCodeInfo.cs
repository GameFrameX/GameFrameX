//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
using Bright.Serialization;
using System.Collections.Generic;
using System.Linq;
using SimpleJSON;

namespace cfg.error
{
    public sealed partial class TbCodeInfo
    {
        private readonly Dictionary<error.EErrorCode, error.CodeInfo> _dataMap;
        private readonly List<error.CodeInfo> _dataList;
        
        public TbCodeInfo(JSONNode _json)
        {
            var count = _json.Children.Count();
            _dataMap = new Dictionary<error.EErrorCode, error.CodeInfo>(count);
            _dataList = new List<error.CodeInfo>(count);
            
            foreach(var _row in _json.Children)
            {
                var _v = error.CodeInfo.DeserializeCodeInfo(_row);
                _dataList.Add(_v);
                _dataMap.Add(_v.Code, _v);
            }
            PostInit();
        }
    
        public Dictionary<error.EErrorCode, error.CodeInfo> DataMap => _dataMap;
        public List<error.CodeInfo> DataList => _dataList;
    
        public error.CodeInfo GetOrDefault(error.EErrorCode key)
        {
            return _dataMap.TryGetValue(key, out var v) ? v : null;
        }
        
        public error.CodeInfo Get(error.EErrorCode key)
        {
            return _dataMap[key];
        }
        
        public error.CodeInfo this[error.EErrorCode key]
        {
            get { return _dataMap[key]; }
        }
    
        public void Resolve(Dictionary<string, object> _tables)
        {
            foreach(var v in _dataList)
            {
                v.Resolve(_tables);
            }
            PostResolve();
        }
    
        public void TranslateText(System.Func<string, string, string> translator)
        {
            foreach(var v in _dataList)
            {
                v.TranslateText(translator);
            }
        }
        
        
        partial void PostInit();
        partial void PostResolve();
    }
}